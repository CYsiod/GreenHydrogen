...
import (
	...
	ast2 "github.com/CYsiod/GreenHydrogen/server/utils/ast"

	"github.com/CYsiod/GreenHydrogen/server/resource/autocode_template/subcontract"
...
func Init(Package string) {
	...
	packageInjectionMap = map[string]astInjectionMeta{
		packageServiceName: {
			...
			importCodeF:  "github.com/CYsiod/GreenHydrogen/server/%s/%s",
			packageNameF: "%s",
...
func filterFile(paths []string) []string {
	...
	for _, path := range paths {
		...
		if ok, _ := skipMacSpecialDocument(fileinfo, path, fileinfo.Name()); ok {
			continue
		}
		np = append(np, path)
	}
...
func skipMacSpecialDocument(srcInfos os.FileInfo, src string, dest string) (bool, error) {
